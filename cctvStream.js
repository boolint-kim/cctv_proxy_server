 ...
 **************************************************************************************/
var cctvObj;
function StreamCctv(cctvid, mapType){
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      // cctvObj = JSON.parse(data);
      cctvObj = JSON.parse(JSON.stringify(data));
    }
  });
  ...
  this.gLocation   = cctvObj.LOCATION;
  ...
  this.getImageHtml = function(){
    if(this.gMovie == 'U'){//유고처리
      ...
      window.open('/map/cctvUgo.jsp', 'UTIC도시교통정보센터', features);
    }else{//정지영상
...
/*      case 'L':
  ...
  this.cctvHTML_Mm = function(){
    return "<object type='application/x-shockwave-flash' data='http://dcmoits.molit.go.kr/js/jwplayer/player.swf' width='100%' height='" + this.gHeight + "' bgcolor='#000000' id='myElement' name='myElement' tabindex='0'>"
      + "<param name='allowfullscreen' value='true'>"
      + "<param name='allowscriptaccess' value='always'>"
      + "<param name='seamlesstabbing' value='true'>"
      + "<param name='wmode' value='opaque'>"
      + "<param name='flashvars' value='netstreambasepath=http://wcmoits.molit.go.kr/NEW/view/cctv.do&id=mediaplayer&provider=rtmp&streamer=rtmp://118.46.175.150/live&file=/" + this.gId + ".stream&autostart=true&wmode=opaque&controlbar.position=none&logo.file=/NEW/images/sub/lmenu_title_3.png'></object>"
    ;
  ...
  this.cctvHTML_y = function(){   
    return "<object id='Player' width='" + this.gWidth + "' height='" + this.gHeight + "' codeBase='http://its.yeosu.go.kr/AMC.cab' classid='CLSID:745395C8-D0E1-4227-8586-624CA9A10A8D'>"       
      ...
      + "<param name='MediaURL' value='http://112.164.152." + this.gCctvIp + "/axis-cgi/mjpg/video.cgi?resolution=4CIF&amp;camera=" + this.gCh + "'>"
      + "<param name='AutoStart' value='-1'>"
      + "<param name='UIMode' value='none'>"
  ...
  this.cctvHTML_Seoul = function(){
    ...
    return "<object id='mov' classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase='http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0' width='320' height='240'>"
      ...
      + "<param name='flashvars' value='file="+ this.gId +".stream&streamer=rtmp://210.179.218."+ this.gCh +":1935/live&bufferlength=3&autostart=true&stretching=exactfit&fullscreen=true&image=http://www.utic.go.kr/contents/js/img/320240-5.gif&skin=http://www.utic.go.kr/contents/js/img/skin_black30live.swf&frontcolor=FFFFFF&lightcolor=00AEEF&backcolor=111111' />"
      + "<embed src='http://www.utic.go.kr/contents/js/img/player.swf' width='320' height='240'  name='movie' flashvars='file="+ this.gId +".stream&streamer=rtmp://210.179.218."+ this.gCh +"/live/&controlbar=none&autostart=true&stretching=exactfit&duration=&image=play.png' style='border:0px solid #c5d6ff' type='application/x-shockwave-flash' pluginspage='http://www.macromedia.com/go/getflashplayer' />"
      + "</object>";
...
function playGgCctvWithProxy(srcFetchUrl, cctvIp, callbackFunc, errorFunc) {
  $.ajax({
    url: srcFetchUrl + '?cctvIp=' + cctvIp,
    async: false,
    success: callbackFunc(data),
    error: errorFunc(error, xhr, code)
  });
